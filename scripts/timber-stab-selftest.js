/*
 * timber-stab-selftest.js: the Timber's stabiliser, acro, rudder, flaps and
 * surface readback, in the air and on its wheels, in Node.
 *
 * cub-stab-selftest.js's cases with the Timber's throws and acro rates,
 * then what the flaps add: the notch and its refusals, the flaps' travel
 * time, a reset keeping the notch and an airframe change stowing it,
 * Stabilised flying level on full flaps, and Acro holding its attitude
 * while the flaps come down. The same stabiliser as the other fixed wings'
 * (src/native/plant_wing.c) with the Timber's own gains, and the
 * Skyhunter's bar: Stabilised flies
 ...
 * renderer reads and their signs. Run with npm run timber:stab.
 *
...
import {
  attitude, must, timberPrelude, timberGroundPrelude, wheelLoads, wingDebug, CUB_AIRFRAME, TIMBER_AIRFRAME, RC_STEP_MS,
...
function throwAt(z, speed) {
  ...
  clockMs = 0;
  timberPrelude(sim, { flaps: sim.flapNotch ?? 0 });
  must(sim.reset(), 'reset');
...
check('full right yaw stick wins over the coordinator: rudder right of half its throw', surfaces()[3] * DEG < -13.5, `rudder ${deg(surfaces()[3])} deg`);
...
check('full right stick rolls at 153 to 198 degrees a second', rolling.p * DEG > 153 && rolling.p * DEG < 198, `${deg(rolling.p)} deg/s`);
const inverted = fly(1, 0, 0, 1.0, 0.25);
...
check('full up stick pitches at 75 to 119 degrees a second', pulling.q * DEG > 75 && pulling.q * DEG < 119, `${deg(pulling.q)} deg/s`);
throwAt(120, 16);
fly(0, 0, 0, 1.0, 1);
fly(0, 0.5, 0, 1.0, 0.3);
...
fly(0, 1, 0, 0.5, 0.02);
sf = surfaces();
check('full up: elevator trailing edge up 20 degrees, ailerons still', close(sf[2], T) && sf[0] === 0 && sf[1] === 0, sf.map(deg).join(' '));
fly(0, 0, 1, 0.5, 0.02);
sf = surfaces();
check('full right yaw: rudder trailing edge right, 27 degrees negative', close(sf[3], -R), sf.map(deg).join(' '));
...
console.log('flaps');
{
  ...
  must(notchOf(2), 'full');
  let t = 0;
  while (sim.e.sim_wing_flaps() < 0.5705837979 && t < 5) {
    fly(0, 0, 0, 0.65, 0.02);
    t += 0.02;
  }
  check('full flaps take the manual\'s 2 s to come down', t > 1.9 && t < 2.1, `${t.toFixed(2)} s to ${deg(sim.e.sim_wing_flaps())} deg`);
  must(notchOf(1), 'half');
  fly(0, 0, 0, 0.65, 2);
  check('and half is 18 degrees', Math.abs(sim.e.sim_wing_flaps() * DEG - 17.98) < 0.01, `${deg(sim.e.sim_wing_flaps())} deg`);
  fly(0, 0, 0, 0.65, 0.02);
  check('with the mix\'s down elevator in the elevator, 16 percent of its travel', Math.abs(surfaces()[2] * DEG + 3.30) < 0.02, `elevator ${deg(surfaces()[2])} deg`);
  must(notchOf(2), 'full');
  must(sim.reset(), 'reset');
  check('a reset keeps the notch and puts the flaps there at once', Math.abs(sim.e.sim_wing_flaps() - 0.5705837979) < 1e-9, `${deg(sim.e.sim_wing_flaps())} deg`);
  must(sim.e.sim_set_airframe(CUB_AIRFRAME), 'cub');
  check('an airframe change raises them', sim.e.sim_wing_flaps() === 0);
  check('and the Cub, which has none, refuses a notch', notchOf(1) !== SIM_OK && notchOf(2) !== SIM_OK && notchOf(0) === SIM_OK);
  must(sim.e.sim_set_airframe(TIMBER_AIRFRAME), 'timber');
  check('back on the Timber the notch is up', sim.e.sim_wing_flaps() === 0);
  must(sim.e.sim_set_ground(0, 0, 0, 1, 0, 0, 0, GROUND_MU, GROUND_E), 'ground off');

  must(sim.e.sim_wing_set_stab(1), 'stabilised');
  sim.flapNotch = 2;
  throwAt(40, 10);
  const slow = fly(0, 0, 0, 0.5, 10);
  ...
  check('at the trim pitch within 4 degrees, slowly', Math.abs(slow.pitch * DEG - 2) < 4 && slow.v < 13, `${deg(slow.pitch)} deg at ${slow.v.toFixed(1)} m/s`);
  const slowBank = fly(1, 0, 0, 0.7, 4);
  ...
  sim.flapNotch = 0;
  throwAt(60, 15);
  const before = fly(0, 0, 0, 0.65, 1);
  must(notchOf(2), 'full');
  const during = fly(0, 0, 0, 0.65, 3, before.endBank);
...
/* The Timber standing at the end of the strip in a mode, then sticks from a
...
function onGround(mode) {
  ...
  must(sim.reset(), 'reset');
  clockMs = 0;
  timberGroundPrelude(sim, { mu: GROUND_MU, e: GROUND_E });
  must(sim.reset(), 'reset');
  timberGroundPrelude(sim, { mu: GROUND_MU, e: GROUND_E });
}
...
for (const [mode, name] of [[1, 'Stabilised'], [2, 'Acro']]) {
  ...
  const sf = surfaces();
  check('full right and up stick held 2 s on the ground is full aileron and elevator, nothing more', Math.abs(held.worstSurf[1] - 30 / DEG) < 1e-9 && Math.abs(held.worstSurf[2] - 20 / DEG) < 1e-9 && sf.every((x) => x === 0), `${held.worstSurf.map(deg).join(' ')}, then ${sf.map(deg).join(' ')}`);
...
for (const [mode, name] of [[1, 'Stabilised'], [2, 'Acro']]) {
  onGround(mode);
  roll(() => [0, 0, 0, 0.3], 2);
  /* Summed step by step, since the Timber turns tighter and quicker than
   * the Cub and comes more than half way round in the 4 s. */
