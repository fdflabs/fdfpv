 ...
 * climb. S6 to S9 are what an old timer is known for, flown in Manual so
 * it is the airframe doing it and not a stabiliser: level its wings by
 * itself from a bank with every stick centred, bank on the rudder the
 * roll stick drives, turn on a held bank, and meet full up elevator with
 * a mush rather than a wing drop, with and without power. S10 to S13 are
 * the phugoid, the prop's torque, a throttle chop and a hand throw; S14 to
 * S16 the gear: standing on its wheels and skid, a take off from the strip
 * with the sticks centred, which its big stabiliser does by lifting the
 * tail by itself, and a landing. S17 holds every other aircraft's recorded
 * hash where it was before this one existed, and S18 flies the Bombshell's
...
const th = JSON.parse(await readFile(join(root, 'tests/bombshell-thresholds.json'), 'utf8'));
...
async function bombshellSim() {
  const sim = await loadSim(wasmBytes);
  if (sim.init(configText) !== SIM_OK) throw new Error('sim_init failed');
  if (sim.e.sim_set_airframe(BOMBSHELL_AIRFRAME) !== SIM_OK) throw new Error('sim_set_airframe failed');
 ...
 * stall guard off, since the guard's 9.5 m/s is a Bombshell's full power,
 * and thrown at its cruise. The performance is still air's, and the
 * Bombshell flies in the airfield's thermals, so these flights start 600 m
 * to the side of the field, clear of every one of them. */
const STILL_AIR = [0, 600, 0, 1, 0, 0, 0];
const slow = { guard: false, speed0: 8.5, start: STILL_AIR };
...
console.log('bombshell gates: the plant against docs/BOMBSHELL-STAGE1.md');
const sim = await bombshellSim();
must(sim.e.sim_wing_set_stab(0), 'sim_wing_set_stab');
check: {
  if (sim.e.sim_airframe() !== BOMBSHELL_AIRFRAME) {
    gate('S0', 'the Bombshell is selected', false, `airframe ${sim.e.sim_airframe()}`, `${BOMBSHELL_AIRFRAME}`);
    break check;
  }

  const s1 = fly(sim, { duty: th.s1_level_75.duty, vzTarget: 0, seconds: 30, ...slow });
  gate('S1', 'cruise: level speed at 75 percent stick', within(s1.v, th.s1_level_75), `${s1.v.toFixed(2)} m/s, sink ${(-s1.vz).toFixed(2)}`, band(th.s1_level_75));

  let stallV = null;
  fly(sim, {
    duty: 0, speed0: 10, seconds: 12, pitchMax: 0.7, pitchMin: -0.3, guard: false, start: STILL_AIR,
  ...
  const s4 = fly(sim, { duty: th.s4_top.duty, vzTarget: 0, seconds: 90, ...slow, speed0: 10 });
   ...
   * proportional, integral and rate loop, since a banked Bombshell levels
   ...
   * the Bombshell, a free flight thermal machine, flies in them: level at cruise with the pitch stick
   ...
   * rises with the air it is in, so the difference is the thermal's. */
  {
    ...
    const pass = (y) => {
      ...
      while (o.s[1] < t19.core[0] + t19.lead && clockMs < 60000) {
        /* Wings level, and on the line: the prop's torque turns the
         * Bombshell left, and the rudder is what brings the nose back. */
  ...
  let rest = null;
  {
    ...
    gate('S14', 'standing on its wheels and skid', rest.pitch >= r14.pitchMin && rest.pitch <= r14.pitchMax && rest.z >= r14.zMin && rest.z <= r14.zMax && rest.tail >= r14.tailMin && rest.tail <= r14.tailMax && hull === 0 && Math.hypot(o.s[4], o.s[5]) < 0.01,
   ...
   * last step on the wheels before 200 ms clear of them. */
  {
    ...
    gate('S15', 'takes off, sticks centred, tail up by itself', lof != null && lof.dist >= t15.distMin && lof.dist <= t15.distMax && lof.v >= t15.vMin && lof.v <= t15.vMax,
   ...
   * the wheels full up to hold the tail down through the roll out. */
  {
    ...
    gate('S16', 'lands on its wheels and skid', touched !== null && touched.vg <= t16.maxTouchSpeed && land.v < 0.05 && Math.abs(land.pitch - rest.pitch) <= t16.pitchTolDeg && hull === 0 && o.loads.slice(0, 3).every((f) => f > 0),
   ...
   * the friction decides, and prints the creep. */
  {
    ...
    gate('S20', 'at idle the engine turns and it stands', rpm >= t20.rpmMin && rpm <= t20.rpmMax && vg <= t20.maxSpeed && d[8] > 0,
      `${rpm.toFixed(0)} rpm, ${d[8].toFixed(3)} N of thrust, ground speed ${(vg * 1000).toFixed(3)} mm/s, crept ${(moved * 1000).toFixed(1)} mm in ${t20.seconds} s`,
      `${t20.rpmMin} to ${t20.rpmMax} rpm, ground speed under ${t20.maxSpeed * 1000} mm/s`);
  }
}
...
const names = ['quad', 'wing', 'sky', 'cub', 'glider', 'bramor', 'bramorChute', 'slowstick', 'timber', 'timberf'];
gate('S17', 'every other aircraft unmoved', names.every((k) => got[k] === u[k]),
  names.map((k) => got[k]).join(', '), names.map((k) => u[k]).join(', '));

const stickRec = await recOf('tests/inputs/bombshell-baseline.rec');
const stickOpts = { ...replayBase, prelude: (s) => bombshellGroundPrelude(s) };
...
if (!findChrome()) {
  ...
  try {
    const out = await runBrowserHarness(`${server.origin}/tests/browser/wing-harness.html?plane=bombshell`);
